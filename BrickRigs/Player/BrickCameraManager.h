...
class BRICKRIGS_API ABrickCameraManager : public APlayerCameraManager
{
	...
	uint8 bFadeHUD : 1;
	// Whether DOF is currently being controlled by us
	uint8 bIsUsingDOF : 1;
	// ~Variables
...
public:
	...
	virtual void UpdatePhotographyPostProcessing(FPostProcessSettings& InOutPostProcessing) override;
...
public:
	// ~FOV
	// Calculates the effective horizontal and vertical FOV for the given FOV value
	FVector2D GetEffectiveFOV(const float FOV) const;
	// Returns the effective FOV of last frame
	FVector2D GetCachedEffectiveFOV() const
	{
		return GetEffectiveFOV(GetCameraCachePOV().FOV);
	}
...
private:
	...
	void UpdateFocalDistance(float DeltaTime, FTViewTarget& OutVT);
};
