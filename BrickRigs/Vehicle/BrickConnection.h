...
struct FConnectorField
{
	...
	{
		float MaxConnectorDistSq = 0.f;
		for (const auto& ConnectorField : InConnectors)
		{
			auto CheckExtrema = [&](int32 X, int32 Y)
			{
				const FTransform ConnectorFieldTM = ConnectorField.GetConnectorTransform();
				const FVector ExtremaLocation = ConnectorField.GetNobLocation(ConnectorFieldTM, X, Y);
...
private:
	...
	void EvalShape(float Val, FFloatInterval& OutRange) const
	{
		...
		case EConnectorShape::Circle:
			{
				const float CircleVal = EvalCircle(Val) * 0.5f;
...
public:
	...
	// Get the brick relative connector transform
	FTransform GetConnectorTransform(const FTransform& BrickTransform) const
	{
		// IMPORTANT: Don't apply the brick scale to the transform
		return GetConnectorTransform() * FTransform(BrickTransform.GetRotation(), BrickTransform.GetLocation());
	}
	...
	{
		const float SpacingDist = GetSpacing();
...
public:
	...
	{
		FBrickConnectionParams OutParams = *this;
...
public:
	...
	void InitializeConnection(UBrick* InBrick0, UBrick* InBrick1, const FBrickConnectionParams& InParams, bool bInIsDynamic);
...
public:
	...
	void UpdateWheel(float DeltaTime, float InSteeringRatio, float InBrakeRatio, bool bInHasBrake, bool bInHandBrake);
