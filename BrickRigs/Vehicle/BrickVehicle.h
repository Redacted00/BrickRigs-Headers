...
// Actor
class ABrickVehicleDownloadReplicator;
// Components
...
UCLASS()
class BRICKRIGS_API ABrickVehicle : public APawn, public IBrickPawnInterface, public IViewTargetInterface, public IGenericTeamAgentInterface, public ISpawnPointInterface, public IFireInterface, public IWinchBrickInterface, public IContactModifyInterface, public IProjectileSeekingInterface
{
	GENERATED_BODY()
...
private:
	...
	// Actor instance used to replicate the download progress of the vehicle
	UPROPERTY(Transient)
	ABrickVehicleDownloadReplicator* DownloadReplicator;
	...
	UPROPERTY(Transient, Replicated)
	FRepBrickConnectionDamage RepBrickConnectionDamage;
	// Contains damaged connections that are waiting to be sent to the server
	FBrickConnectionDamageBitfield ClientBrickConnectionsToBreak;
	...
	// Clusters of parts that replicate movement together
	TArray<FVehicleRepMoveCluster> RepMoveClusters;

	// Whether the vehicle is registered as a heat seeking target
	uint8 bIsHeatSeekingTarget : 1;
...
private:
	...
	// Registers or unregisters the vehicle as a heat seeking target as needed
	void UpdateIsHeatSeekingTarget(bool bIsEndPlay = false);
...
public:
	...
	// ~Connections
	// Returns the number of static brick connections
	int32 GetNumBrickConnections() const
	{
		return BrickConnections.Num();
	}

	// Returns the connection at the given index
	UBrickConnection* GetBrickConnectionByIndex(const int32 Index) const;
...
public:
	...
	{
		BrickConnectionsToBreak.Append(InConnections);
	}

	// Adds a single connection to the queue
	void QueueBrickConnectionToBreak(UBrickConnection* InConnection)
	{
		BrickConnectionsToBreak.Add(InConnection);
	}
...
public:
	...
	// ~Bricks
	// Adds or removes a brick from the replication list
	void UpdateBrickReplicated(UBrick* Brick, const bool bNewReplicated);
...
public:
	...
	// To be called when a replicated brick movement was added or changed
	void SetBrickReplicatedMovementTarget(const FRepBrickMovement& BrickMovement);

	// To be called when a replicated brick movement target was removed
	void RemoveBrickReplicatedMovementTarget(const FRepBrickMovement& BrickMovement);

	// Calls the RPC to send a movement update to the server
	void SendMovementToServer();

	// Called from the player controller RPC to update movement
	void ServerUpdateMovement(const TArray<FRepBrickMovementState>& MovementSates, const float OwnerTimestamp);

	// Called from the player controller RPC to update connection damage
	void ServerUpdateConnectionDamage(const FBrickConnectionDamageBitfield& ConnectionDamage);

	// Called from the player controller RPC to revert to the last replicated connection damage
	void ClientRevertConnectionDamage();

private:
	// Gathers the replicated vehicle movement state
	void GatherVehicleMovement();

	// Returns the rep move cluster for which the given brick is the root
	const FVehicleRepMoveCluster* GetRepMoveCluster(const UBrick* ClusterRoot) const;

	// Generates a replicated movement state for a brick
	FRepBrickMovementState GetRepBrickMovementState(const FBrickEditorObjectID& BrickID, const UBrick* Brick) const;

	// To be called whenever the vehicle might receive or lose physics authority
	void UpdateHasPhysicsAuthority();

	// Updates the replication movements targets
	void UpdateBrickReplicatedMovementTargets();
	// ~Replication
...
public:
	...
	// Try to find a brick by its index
	template <class T = UBrick>
	T* GetBrickByIndex(const int32 Index) const
	{
		return VehicleComponent->GetBrickEditorObjectByIndex<UBrick, T>(Index);
	}
