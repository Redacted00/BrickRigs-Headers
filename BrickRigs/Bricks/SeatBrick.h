...
class BRICKRIGS_API USeatBrick : public UBrick
{
	...
	inline const static FVector InvalidExitLocation = FVector(MAX_FLT);
	...
	uint8 bIsFiring : 1;
	// Whether to force the third person camera
	uint8 bUseThirdPersonCamera : 1;
	// True in the PreRepairBrick function
	uint8 bIsRepairingSeat : 1;
	// ID of the last view target brick, could be a camera, seat (first person) or none (third person)
	FBrickEditorObjectID ViewTargetBrickID;
	// ~Variables
...
public:
	...
	{
		return HasCustomExitLocation() ? TOptional(GetWorldTransform().TransformPositionNoScale(ExitLocation)) : TOptional<FVector>();
	}
...
public:
	// ~View Target
	// Changes the view target
	void SetViewTargetBrickID(const FBrickEditorObjectID& InID);

	FBrickEditorObjectID GetActiveViewTargetBrickID() const
	{
		return bUseThirdPersonCamera ? nullptr : ViewTargetBrickID;
	}

	// Cycles between available view targets
	void CycleViewTarget(int32 Dir);
	...
	{
		return GetActiveViewTargetBrickID().IsNull();
	}

	bool IsInFirstPersonCamera() const
	{
		return GetActiveViewTargetBrickID() == this;
	}
