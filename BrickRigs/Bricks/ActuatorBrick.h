...
public:
	// ~Properties
	// True if the actuator moves angularly, false if it is a linear actuator
	UPROPERTY(EditDefaultsOnly, Category = Actuator)
...
class BRICKRIGS_API UActuatorBrick : public UBrick
{
	...
	// Time the actuator state should be synced again, -1 while disabled
	float ActuatorStateSyncTime;
...
private:
	...
	// Returns whether the local client has authority over controlling the actuator
	bool HasActuatorAuthority() const;
	...
	// To be called whenever the actuator state has changed
	void OnActuatorStateChanged(const FActuatorState& PrevState);
