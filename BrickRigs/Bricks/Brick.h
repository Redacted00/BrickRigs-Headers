...
static int32 GVisualizeFluidDynamics = 0;
static int32 GDisableDetachedBrickCollision = 0;
#endif
...
protected:
	...
	uint16 bHasMovementReplicationBeenUpdated : 1;
...
public:
	struct FTickFluidDynamicsParams
	{
		...
		const FRepPhysicsMovement& RepMovement;
	};
...
public:
	...
	// Used to get the world transform of the connector at the given index
	FTransform GetConnectorTransform(int32 ConnectorIndex) const;

	// Get the relative location and rotation of a connector field
	FTransform GetConnectorRelativeTransform(int32 ConnectorIndex) const;

	// Get the current world transform of a connector field
	FTransform GetConnectorWorldTransform(int32 ConnectorIndex) const;
...
public:
	// Get all part roots that are rigidly connected to this one and should replicate movement together
	void GatherMovementReplicationCluster(const UBrick* RootBrickPartRoot, UBrick*& OutRoot, uint16& OutPriority)
	{
		check(IsBrickPartRoot());

		auto MaxMass = -1.f;
		GatherReplicationClusterChildPartsInternal(RootBrickPartRoot, OutRoot, OutPriority, MaxMass);
	}

private:
	// Used for recursion
	void GatherReplicationClusterChildPartsInternal(const UBrick* RootBrickPartRoot, UBrick*& OutRoot, uint16& OutPriority, float& MaxMass)
	{
		check(IsBrickPartRoot());

		bHasMovementReplicationBeenUpdated = true;
		...
		if (this == RootBrickPartRoot)
		{
			...
			OutRoot = this;
			MaxMass = MAX_flt;
			// Make sure the root brick is always replicated
			OutPriority = MAX_uint16;
		}
		else
		{
			...
			// Update the priority, make sure not to replicate bricks that aren't attached to anything
			OutPriority = FMath::Max(OutPriority, GetNumConnectedBricks() > 0 ? PartRootParams->MovementReplicationPriority : static_cast<uint16>(0));
		}
		...
		for (const auto* Connection : PartRootParams->ExternalPartConnections)
		{
			...
			// Add parts that are connected through a non free connection
			if (OtherPartRoot->GetVehicle() == GetVehicle() && Connection->CanJoinMovementReplication() && !OtherPartRoot->bHasMovementReplicationBeenUpdated)
			{
				OtherPartRoot->GatherReplicationClusterChildPartsInternal(RootBrickPartRoot, OutRoot, OutPriority, MaxMass);
			}
		}
	}
...
public:
	...
	void SetPartRootChildren(const TArray<UBrick*>& NewChildren, const TArray<UPhysicsConstraintConnection*>& NewExternalPartConnections, TArray<UPhysicsConstraintConnection*>& OutConnectionsToUpdate, bool bIsInitializingConnections, bool bInitialzedPartRoot, bool bShouldSimulatePhysics, bool bBricksAddedOrRemoved);
...
private:
	...
	void UpdatePartRoot(bool bInInitializeBricks, bool bInIsConnectedToRoot, bool bInHasAnyFuel, int32 NumTotalBricks, bool& bOutHasAnyFluidDynamicElements);
...
protected:
	...
	{
		Params.FluidDynamicElements.Surfaces.Add(ShouldUseFluidDynamicLiftSurface());
	}
...
public:
	...
	bool HasMovementReplicationBeenUpdated() const
	{
		return bHasMovementReplicationBeenUpdated;
	}
	...
	void PreGatherConnectedBricks()
	{
		...
		bHasMovementReplicationBeenUpdated = false;
	}
	...
	void UpdateClusterRoot(bool bInInitializeBricks, bool bInIsConnectedToRoot, int32 NumTotalBricks, TArray<FBrickEditorObjectID>& ClusterRootBricks, TArray<FBrickEditorObjectID>& ClusterRootBricksWithFluidDynamics);
...
protected:
	// Get the parent material used for the brick
	UMaterialInterface* GetBrickBaseMaterial() const;
	...
	void AssignBrickMaterialCustomPrimitiveData(T Container, const FTransform& Transform, const FTransform& RelativeTransform = FTransform::Identity) const
	{
		...
		else
		{
			...
			auto NewColor = IsBrickBurnt() ? FLinearColor::White : BrickColor.ToLinearRGB();

			// If the material doesn't have a diffuse map it means the color parameter is used as the default color
			// So we want to interpolate the color to the paint color in that case
			if (NewColor.A < 1.f - KINDA_SMALL_NUMBER && BrickMaterial && !BrickMaterial->bHasDiffuseMap)
			{
				// NOTE: Don't interpolate alpha channel
				NewColor.R = FMath::Lerp(BrickMaterial->DiffuseColor.R, NewColor.R, NewColor.A);
				NewColor.G = FMath::Lerp(BrickMaterial->DiffuseColor.G, NewColor.G, NewColor.A);
				NewColor.B = FMath::Lerp(BrickMaterial->DiffuseColor.B, NewColor.B, NewColor.A);
			}
...
public:
	...
	UBrickPhysicalMaterial* GetBrickPhysMaterial(bool bAllowOverrideMaterial = true) const;
	// ~Material
