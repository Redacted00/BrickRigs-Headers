...
struct FWinchAttachTarget
{
	...
	{
		return Component.IsValid() ? Cast<IWinchBrickInterface>(Component->GetOwner()) : nullptr;
	}
	...
	TOptional<FVector> GetWorldLocation() const
	{
		if (bIsAttached)
		{
			if (Component.IsValid() && Component->IsRegistered())
			{
				return Component->GetComponentTransform().TransformPosition(RelativeLocation);
			}
