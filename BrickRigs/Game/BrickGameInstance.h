...
class BRICKRIGS_API UBrickGameInstance : public UGameInstance, public FTickableGameObject
{
	...
	DECLARE_MULTICAST_DELEGATE_OneParam(FOnJoinSessionStateChanged, EJoinSessionState);
	...
	DECLARE_MULTICAST_DELEGATE_TwoParams(FOnUGCUploadComplete, EUGCUploadResult, const FOnlineUGCItemIdWrapper&);
	...
	FDelegateHandle DelegateHandle_QueryUGC;
	FDelegateHandle DelegateHandle_UploadItem;
	...
	// The currently active UGC query
	TSharedPtr<FOnlineUGCQueryState> UGCQueryState;
	// Cached UGC type being queried for
	EUGCType CurrentUGCQueryType;
	// The current UGC upload task
	TSharedPtr<FUGCUploadState> UGCUploadState;
...
public:
	...
	// Cancels the current UGC query, returns true upon success
	bool CancelQueryUGC();
	// Starts a new UGC upload task
	bool StartUGCUpload(const FUGCFileInfo& FileInfo, EUGCUploadFlags Flags);
...
private:
	...
	EOnlineUGCQueryType ConvertUGCQueryTypeToOnlineQueryType(EUGCQueryType QueryType) const;
	// Converts a sort method to an online sort method
	EOnlineUGCSortMethod ConvertUGCSortMethodToOnlineSortMethod(EUGCSortMethod SortMethod) const;
	...
	void OnUGCQueryCompleteInternal(const TArray<FUGCQueryResult>& Results, uint32 NumTotalResults, uint32 NumPages);
	// Callbacks for the UGC interface
	void OnUGCQueryComplete(bool bSuccess);
	void OnUGCUploadComplete(bool bSuccess);
