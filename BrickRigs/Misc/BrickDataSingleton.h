...
// Contains info about a brick class that has been removed
USTRUCT(BlueprintType)
struct FLegacyBrickEditorObjectClass
{
	GENERATED_BODY()

	// Suffix of this specific class
	UPROPERTY(EditAnywhere)
	FString Suffix;
};

USTRUCT(BlueprintType)
struct FLegacyBrickEditorObjectClassCategory
{
	GENERATED_BODY()

	// Every class name in this category has to start with this prefix
	UPROPERTY(EditAnywhere)
	FString Prefix;
	// The class to use instead of the deprecated class
	UPROPERTY(EditAnywhere)
	TSubclassOf<UBrickEditorObjectStaticInfo> ReplacementClass;
	UPROPERTY(EditAnywhere)
	TArray<FLegacyBrickEditorObjectClass> LegacyClasses;
};

struct FLegacyBrickEditorObjectClassID
{
	// ~Constructor
	FLegacyBrickEditorObjectClassID(const uint8 CategoryIndex = MAX_uint8, const uint8 ClassIndex = MAX_uint8)
...
protected:
	...
	// List of deprecated brick editor object classes
	UPROPERTY(EditDefaultsOnly, Category = Bricks)
	TArray<FLegacyBrickEditorObjectClassCategory> LegacyBrickEditorObjectClasses;
...
public:
	...
	// Try to find a replacement for a deprecated brick class
	FLegacyBrickEditorObjectClassID ResolveBrickEditorObjectClass(const FString& InClassName) const;
	// Returns the category associated with a class id, which has to be valid!
	const FLegacyBrickEditorObjectClassCategory& GetLegacyBrickEditorObjectClassCategory(const FLegacyBrickEditorObjectClassID& ClassId) const;
	// Returns the class info associated with a class id, which has to be valid!
	const FLegacyBrickEditorObjectClass& GetLegacyBrickEditorObjectClass(const FLegacyBrickEditorObjectClassID& ClassId) const;
	// ~Bricks
};
