...
public:
	...
	virtual void SetIsFiring(bool bNewFiring)
	{
	}
...
public:
	...
	virtual bool NeedsThumbnailMID() const { return false; }
	// Allows the item to customize its thumbnail capture MID
	virtual void UpdateThumbnailMID(UMaterialInstanceDynamic* InMID) const
	{
	}
	...
	virtual void SetNumItems(int32 Num);
	...
	virtual int32 GetNumDefaultItems() const;
	// ~Stackables
	...
	// Called from the soldier whenever the sight needs to be updated
...
protected:
	...
	virtual void GetInteractionOptions(ABrickPlayerController* PC, FInteractionOptions& OutOptions);
...
private:
	...
	// Removes the item from the phusics replication manager
	void RemovePhysicsReplicationTarget();
