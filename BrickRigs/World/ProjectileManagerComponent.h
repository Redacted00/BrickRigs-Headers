...
public:
	...
	bool SpawnProjectile(UFirearmComponent* Firearm, const FVector& Location, const FRotator& Rotation, const FVector& Velocity, const bool bReplicateProjectile, const float DeltaTimeCorrection, const bool bActuallySpawn, const bool bMergeProjectile);
	...
	void SpawnFlybyEffect(USoundBase* FlybySound, const FVector& Location, float Volume, float Pitch);
...
public:
	...
	void SpawnSoundEffect(USoundBase* Sound, const FVector& Location, float Volume = 1.f, float Pitch = 1.f);
	// Spawns a new particle system or uses one from the pool
	UParticleSystemComponent* SpawnEmitter(UParticleSystem* Template, AActor* Actor, USceneComponent* AttachParent, const FName& SocketName, const FVector& Location, const FRotator& Rotation, const float ProjectileRadius, const bool bUpdateLOD);
