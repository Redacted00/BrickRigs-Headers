...
template <>
struct TStructOpsTypeTraits<FWorldSetupParams> : TStructOpsTypeTraitsBase2<FWorldSetupParams>
{
	enum
	{
...
struct FMatchSettings : public FBrickStructPropertyInterface
{
	...
	constexpr static int32 DefaultMaxBricksPerVehicle = 1000;
	constexpr static float DefaultMaxVehicleSize = 10000.f;
	constexpr static float DefaultMaxVehicleMass = 100000.f;
	constexpr static int32 DefaultMaxVehiclesPerPlayer = 4;
	inline const static auto DefaultVehicleRemovalDelay = FTimespan::FromMinutes(1);
...
public:
	// ~Vehicles
	// What types of vehicles are allowed to be spawned
	UPROPERTY(EditAnywhere, BlueprintReadWrite)
	EMatchVehicleFilter VehicleFilter = EMatchVehicleFilter::PublicOnline;
	...
	UPROPERTY()
	uint16 MaxBricksPerVehicle = DefaultMaxBricksPerVehicle;
	// Maximum mass vehicles are allowed to have to be spawned
	UPROPERTY(EditAnywhere, BlueprintReadWrite)
	float MaxVehicleMass = DefaultMaxVehicleMass;
	// Maximum size in any direction
	UPROPERTY(EditAnywhere, BlueprintReadWrite)
	float MaxVehicleSize = DefaultMaxVehicleSize;
	// Maximum number of vehicles a player is allowed to spawn
	UPROPERTY(EditAnywhere, BlueprintReadWrite)
	uint8 MaxVehiclesPerPlayer = DefaultMaxVehiclesPerPlayer;
	// Multiplies vehicle prices
	UPROPERTY(EditAnywhere, BlueprintReadWrite)
	float VehiclePriceFactor = 0.1f;
	// Time until empty vehicles are removed from the map
	UPROPERTY(EditAnywhere, BlueprintReadWrite)
	FTimespan VehicleRemovalDelay = DefaultVehicleRemovalDelay;
	// ~Vehicles
...
private:
	...
	static void GetVehicleFilterItems(const FBrickPropertyContainer& Container, TArray<FEnumPropertyItem>& OutItems);
};
