...
private:
	...
	const TArray<TWeakObjectPtr<>> ContainerObjects;
	// Whether the property should appear as enabled
	bool bIsEnabled = true;
	// Whether the property is shown as read only
	bool bIsReadOnly = false;
	// Color style to use
	EBrickUIColorStyle ColorStyle = EBrickUIColorStyle::Default;
	// Used for combo box properties, max number of items to display in a combo box
	int32 MaxComboBoxListItems = INDEX_NONE;
	// Max amount of items to display per row
	int32 MaxComboBoxItemsPerRow = 0;
	// Changed event that has yet to be committed
	TSharedPtr<FBrickPropertyChangedEvent> PendingChangedEvent;
	// Allows overriding the property orientation
	TOptional<TEnumAsByte<EOrientation>> OrientationOverride;
	// ~Variables

public:
	// ~Constructor
	FBrickPropertyEditInfo(const FBrickPropertyInstance& InPropertyInstance, const FText& InDisplayName, const TArray<TWeakObjectPtr<>>& InContainerObjects)
		: FBrickPropertyInstance(InPropertyInstance), DisplayName(InDisplayName), ContainerObjects(InContainerObjects)
	{
	}
	...
	auto& SetColorStyle(const EBrickUIColorStyle InColorStyle)
	{
		ColorStyle = InColorStyle;
		return *this;
	}

	auto GetColorStyle() const
	{
		return ColorStyle;
	}
	...
	FBrickPropertyEditInfo& SetOrientationOverride(const EOrientation Orientation)
	{
		OrientationOverride = Orientation;
		return *this;
	}

	const auto& GetOrientationOverride() const
	{
	...
	// Whether this property can be copied by value
	bool CanSetPropertyValueByCopy() const;
...
bool FBrickPropertyEditInfo::SetPropertyValueInternal(ABasePlayerController* PC, EValueChangedEventType EventType, SetterType SetterFunction)
{
	...
	const auto NumContainers = ContainerObjects.Num();
	...
	for (auto i = NumContainers - 1; i >= 0; --i)
	{
		const auto ContainerObject = ContainerObjects[i].Get();
	...
	if (PendingChangedEvent->GetNumObjects() > 0)
	{
		...
		const auto ChangedEvent = *PendingChangedEvent.Get();
