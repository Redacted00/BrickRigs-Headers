...
public:
	...
	// Outputs the bound value, returns true upon success
	bool GetValue(const FBrickPropertyContainer& Container, T& OutValue) const
	{
		if (Delegate.IsBound())
		{
			OutValue = Delegate.Execute(Container);
			return true;
		}
		if (Value.IsBound())
		{
