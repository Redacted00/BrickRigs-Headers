...
struct FBrickEditorObjectEditorParams
{
	...
	// The object that is the move root to this one, null if this is the move root
...
class BRICKRIGS_API UBrickEditorObject : public UObject, public IBrickPropertyInterface
{
	...
	// Whether a deprecated class has been replaced by this object
	uint8 bReplacedDeprecatedBrickEditorObject : 1;
...
public:
	...
	static UBrickEditorObject* CreateBrickEditorObject(
		...
		const FString& InReplacedClassName,
		bool bDeferPostLoad
	);
	// To be called after properties have been loaded
	static void PostLoadBrickEditorObject(UBrickEditorObject* InObject, FBrickRigsSaveVersion Version, const FBrickEditorReferenceResolver* ReferenceResolver = nullptr);
...
private:
	...
	void InitializeBrickEditorObject(UClass* InStaticInfoClass, const FBrickEditorObjectID& InObjectID, EBrickEditorObjectContext InEditorContext, bool bInHasBeenRecycled, bool bInCreatedAsMirrored, const FString& InReplacedClassName);

	// Uninitializes and cleans up the object
	void UninitializeBrickEditorObjectWrapper();

	// Gives the object a chance to setup its default values, called before it is loaded
	virtual void SetupBrickEditorObjectDefaults()
	{
	}

	// Called after the object has been created and properties have been loaded
	virtual void PostLoadBrickEditorObject(FBrickRigsSaveVersion Version, const FBrickEditorReferenceResolver* ReferenceResolver)
	{
	}
...
public:
	...
	void BeginEditorMove(UBrickEditorObject* InMoveRoot, bool bInIsMirrored);
	...
	{
		return EditorParams.IsValid() && !EditorParams->MoveRoot.IsValid();
	}
	...
	void SetMoveRootTransform(const FVector& NewLocation, const FRotator& NewRotation);

	// Returns the expected transform when the move transform is applied
	auto GetExpectedMoveTransform(const FTransform& MoveTransform) const
	{
		return EditorParams.IsValid() && EditorParams->MoveRoot.IsValid() ? GetRelativeTransform() * MoveTransform : MoveTransform;
	}

	// Returns the effective transform of the current move
	auto GetMoveTransform() const
	{
		return EditorParams.IsValid() && EditorParams->MoveRoot.IsValid() ? GetRelativeTransform() * EditorParams->MoveRoot->GetRelativeTransform() : GetRelativeTransform();
	}

	// Gives the object a chance to check if required properties are mirrored from the other object
	virtual bool AreBrickPropertiesMirroredFrom(UBrickEditorObject* OtherObject, EBrickEditorMirrorMode MirrorMode) const
	{
		return true;
	}

	// Get the transform to apply when the brick is mirrored
	void GetMirroredTransform(FVector& OutLocation, FQuat& OutRotation, EBrickEditorMirrorMode MirrorMode) const;
...
public:
	...
	// ~Serialization
	// Whether this object has replaced a removed class
	bool HasReplacedDeprecatedBrickEditorObject() const
	{
		return bReplacedDeprecatedBrickEditorObject;
	}
	...
	// Called when the object has been created as a replacement for a legacy class
	virtual void ResolveDeprecatedBrickEditorObjectClass(const FString& LegacyClassName)
	{
	}
