...
public:
	...
	const static int32 MaxNumBrickEditorObjects = 50000;

private:
	DECLARE_MULTICAST_DELEGATE_OneParam(FOnUGCDownloadComplete, bool);
	DECLARE_MULTICAST_DELEGATE_TwoParams(FOnUGCItemLoaded, bool, TArray<uint8>*);
	...
	TSharedPtr<FOnlineUGCItemId> DownloadingItemId;
	// Whether the UGC item is currently being dowloaded
	uint8 bIsDownloadingItem : 1;
...
public:
	...
	bool LoadItem(const FUGCFileInfo& FileInfo, bool bImport);
	// Load the creation from disk
	bool LoadFromFile(const FString& FileName, bool bImport);
	// Load the creation from the given archive
	bool LoadFromArchive(FArchive& Ar, TArray<uint8>& Data, bool bImport, bool bFromUndo);

private:
	// Called after successfully loading all objects and properties
	virtual bool PostLoadFromArchive(FArchive& Ar, const FBrickEditorSaveHeader& Header, bool bImport, const FBrickEditorReferenceResolver& ReferenceResolver)
	{
		return true;
	}

	// Loads objects contained in the archive
	bool LoadObjectsFromArchive(TArray<UBrickEditorObject*>& OutObjects, FArchive& Ar, const FBrickEditorSaveHeader& Header, EBrickEditorObjectCreationContext CreationContext, EBrickEditorMirrorMode MirrorMode);
	// Internal version used to optimize the legacy and non legacy file paths
	template <bool bIsLegacyFile>
	bool LoadObjectsFromArchiveInternal(TArray<UBrickEditorObject*>& OutObjects, FArchive& Ar, const FBrickEditorSaveHeader& Header, EBrickEditorObjectCreationContext CreationContext, EBrickEditorMirrorMode MirrorMode);
...
public:
	...
	// Get the current progress of the item download, returns true if the item is being downloaded
	bool GetItemDownloadProgress(float& OutProgress) const;

private:
	// Sets the downloading flag and broadcasts the delegate
	void SetIsDownloadingItem(bool bNewIsDownloading);
	// Callback for the online UGC interface
	void OnDownloadUGCItemComplete(const TSharedRef<FOnlineUGCItemId>& ItemId, bool bSuccess, const FString& InstallDirectory);
...
public:
	...
	// Get the mirrored counterpart to the given object
	UBrickEditorObject* GetMirroredBrickEditorObject(UBrickEditorObject* InObject, EBrickEditorMirrorMode MirrorMode) const;
	// ~Get Object
...
private:
	// Can be used to remap a deprecated class name to a new class
	virtual FString ResolveDeprecatedBrickEditorObjectClass(const FString& InClassName)
	{
		return FString();
	}
...
private:
	...
	UBrickEditorObject* CreateBrickEditorObject(UClass* InStaticInfoClass, EBrickEditorMirrorMode MirrorMode, const FString& ReplacedClassName = "");
	...
	void PostCreateBrickEditorObjects(const TArray<UBrickEditorObject*>& NewObjects, EBrickEditorObjectCreationContext CreationContext, FBrickRigsSaveVersion Version = BR_SAVE_VERSION, const FBrickEditorReferenceResolver* ReferenceResolver = nullptr);
