...
struct FBrickEditorUndoStep
{
	...
	{
		for (int32 StepIdx = StartIdx; StepIdx >= 0; --StepIdx)
		{
			const FBrickEditorUndoStep& OtherStep = List[StepIdx];
			for (int32 FlagIdx = 0; FlagIdx < static_cast<int32>(EBrickEditorUndoFlag::Max); ++FlagIdx)
			{
				const EBrickEditorUndoFlag Flag = static_cast<EBrickEditorUndoFlag>(FlagIdx);
