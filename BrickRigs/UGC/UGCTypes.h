...
// Wrapper used to handle item id references more easily
USTRUCT(BlueprintType)
struct FOnlineUGCItemIdWrapper
{
	GENERATED_BODY()

	// ~Constructor
	FOnlineUGCItemIdWrapper(const TSharedPtr<FOnlineUGCItemId>& InItemId = nullptr)
		: ItemId(InItemId)
	{
	}
	...
	const TSharedPtr<FOnlineUGCItemId>& operator->() const
	{
		return ItemId;
	}

	const FOnlineUGCItemId& operator*() const
	{
		check(ItemId.IsValid());
		return *ItemId;
	}

	// Implicit shared pointer conversion
	operator const TSharedPtr<FOnlineUGCItemId>&() const
	{
		return ItemId;
	}

	// Explicit shared ref conversion
	TSharedRef<FOnlineUGCItemId> ToSharedRef() const
	{
		return ItemId.ToSharedRef();
	}
	...
	void operator=(const TSharedPtr<FOnlineUGCItemId>& InItemId)
	{
		ItemId = InItemId;
	}

	bool operator==(const FOnlineUGCItemIdWrapper& Other) const
	{
		return IsSameId(ItemId, Other);
	}
	...
	FString ToString() const
	{
		if (IsValid())
		{
			return ItemId->GetType().ToString() + '_' + ItemId->ToString();
		}
		return "None";
	}

	// Sets the id from the online subsystem name and id string
	void FromString(const FName& OnlineSubsystemName, const FString& IdStr);

	// Sets the id from a string containing the subsystem name and id string in the format Steam_0123456
	void FromString(const FString& Str);

	bool ExportTextItem(FString& ValueStr, const FOnlineUGCItemIdWrapper& DefaultValue, UObject* Parent, int32 PortFlags, UObject* ExportRootScope) const
	{
		ValueStr = ToString();
		return true;
	}
	...
	// Compare as wrapper vs ref
	static bool IsSameId(const FOnlineUGCItemIdWrapper& LHS, const TSharedRef<FOnlineUGCItemId>& RHS)
	{
		return IsSameId(RHS, LHS.ItemId);
	}

	// Compare as wrapper vs ptr
	static bool IsSameId(const FOnlineUGCItemIdWrapper& LHS, const TSharedPtr<FOnlineUGCItemId>& RHS)
	{
		return IsSameId(RHS, LHS.ItemId);
	}

	// Compare as shared refs
	static bool IsSameId(const TSharedRef<FOnlineUGCItemId>& LHS, const TSharedRef<FOnlineUGCItemId>& RHS)
	{
		return *LHS == *RHS;
	}

	// Compare as shared pointers
	static bool IsSameId(const TSharedPtr<FOnlineUGCItemId>& LHS, const TSharedPtr<FOnlineUGCItemId>& RHS)
	{
		return LHS.IsValid() == RHS.IsValid() && (!LHS.IsValid() || IsSameId(LHS.ToSharedRef(), RHS.ToSharedRef()));
	}

	// Compare a shared ref vs a pointer
	static bool IsSameId(const TSharedRef<FOnlineUGCItemId>& LHS, const TSharedPtr<FOnlineUGCItemId>& RHS)
	{
		return RHS.IsValid() && IsSameId(LHS, RHS.ToSharedRef());
	}

	friend FArchive& operator<<(FArchive& Ar, FOnlineUGCItemIdWrapper& Wrapper);

private:
	// ~Variables
	TSharedPtr<FOnlineUGCItemId> ItemId;
	// ~Variables
};

template <>
struct TStructOpsTypeTraits<FOnlineUGCItemIdWrapper> : TStructOpsTypeTraitsBase2<FOnlineUGCItemIdWrapper>
{
	enum
	{
...
struct FUGCFileInfo : public FPagedListEntry
{
	...
	constexpr static int32 MaxTitleLength = 100;
...
public:
	...
	FUGCFileInfo(EUGCFileType InFileType = EUGCFileType::None, EUGCType InUGCType = EUGCType::None, const FBrickRigsSaveVersion& InVersion = BR_SAVE_VERSION, const FString& InLocalItemId = FString(), const TSharedPtr<FOnlineUGCItemId>& InOnlineItemId = nullptr, const TArray<uint8>& InItemData = {})
		: FileType(InFileType)
		  , UGCType(InUGCType)
		  , Version(InVersion)
		  ...
		  , TimePlayedWeek(0)
		  , TimePlayedTotal(0)
		  , Visibility(EOnlineUGCVisibility::Unlisted)
	{
	}
	...
	static FUGCFileInfo MakeOnlineFile(EUGCType InUGCType, const TSharedRef<FOnlineUGCItemId>& InItemId, const FOnlineUGCQueryResult& QueryResult);
	// ~Factories
...
public:
	...
	{
		return FileType == Other.FileType && UGCType == Other.UGCType && ContentVersion == Other.ContentVersion && LocalItemId == Other.LocalItemId && FOnlineUGCItemIdWrapper::IsSameId(OnlineItemId, Other.OnlineItemId) && ItemData == Other.ItemData;
	}
	...
	const FOnlineUGCItemIdWrapper& GetOnlineItemId() const
	{
		return OnlineItemId;
	}

	void SetOnlineItemId(const TSharedPtr<FOnlineUGCItemId>& InItemId)
	{
...
public:
	...
	// WARNING: This is only meant to be used for replication
	bool operator==(const FUGCFileInfo& Other) const
	{
		// Don't compare the item data, would be too expensive. Only need to re-replicate if the file type has changed (workshop item was converted to an inline item)
		return GetExactFileType() == Other.GetExactFileType();
	}
	...
	void ImportOnlineMetaData(const FString& InMetaData)
	{
		...
		{
			Version = FBrickRigsSaveVersion::FromString(Array[0]);
		}
...
template <>
struct TStructOpsTypeTraits<FUGCFileInfo> : public TStructOpsTypeTraitsBase2<FUGCFileInfo>
{
	enum
	{
		WithNetSerializer = true,
...
struct FUGCQuerySettings
{
	...
	TArray<TSharedRef<FOnlineUGCItemId>> SearchItemIDs;
	int32 Page;
...
struct FUGCPaths
{
	inline static const FString NoPluginName = FString();
